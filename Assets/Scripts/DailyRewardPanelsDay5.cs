...
public class DailyRewardPanelsDay5 : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        if (PlayerPrefs.GetInt("ValueOnceSet3") == 1)
        {
            //do nothing
        }
        else
        {
            if (PlayerPrefs.GetInt("Callchestonce5") == 1)
            {

            }
            else
            {
                PlayerPrefs.SetInt("Callchestonce5", 1);
                ChestClick();
            }
        }
        lastchestopen =  ulong.Parse(PlayerPrefs.GetString("LastChestOpenDay5"));
        if (!isChestReady())
        {
    ...
    public void ChestClick()
    {
        ...
        PlayerPrefs.SetString("LastChestOpenDay5", lastchestopen.ToString());
        ChestButton.interactable = false;
        if (PlayerPrefs.GetInt("ValueOnceSet3") == 1)
        {
    ...
    private bool isChestReady()
    {
       ...
       // Debug.Log(SecondLeft);
        if (SecondLeft < 0)
        {
            ...
            PlayerPrefs.SetInt("ValueOnceSet3", 1);
            return true;
        }
        return false;   
    }
