...
public class DailyRewardPanelsDay4 : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        if (PlayerPrefs.GetInt("ValueOnceSet2") == 1)
        {
            //do nothing
        }
        else
        {
            if (PlayerPrefs.GetInt("Callchestonce4") == 1)
            {

            }
            else
            {
                PlayerPrefs.SetInt("Callchestonce4", 1);
                ChestClick();
            }
        }
        lastchestopen =  ulong.Parse(PlayerPrefs.GetString("LastChestOpenDay4"));
        if (!isChestReady())
        {
    ...
    public void ChestClick()
    {
        ...
        PlayerPrefs.SetString("LastChestOpenDay4", lastchestopen.ToString());
        ChestButton.interactable = false;
        if (PlayerPrefs.GetInt("ValueOnceSet2") == 1)
        {
    ...
    private bool isChestReady()
    {
       ...
       // Debug.Log(SecondLeft);
        if (SecondLeft < 0)
        {
            ...
            PlayerPrefs.SetInt("ValueOnceSet2", 1);
            return true;
        }
        return false;   
    }
