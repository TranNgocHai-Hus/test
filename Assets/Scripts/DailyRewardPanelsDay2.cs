...
public class DailyRewardPanelsDay2 : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        if (PlayerPrefs.GetInt("ValueOnceSet") == 1)
        {
            //do nothing
        }
        else
        {
            if (PlayerPrefs.GetInt("Callchestonce2") == 1)
            {
                //do nothing
            }
            else
            {
                PlayerPrefs.SetInt("Callchestonce2", 1);
                ChestClick();
            }
            
        }
        
        lastchestopen=  ulong.Parse(PlayerPrefs.GetString("LastChestOpenDay2"));
        if (!isChestReady())
        {
    ...
    public void ChestClick()
    {
        ...
        PlayerPrefs.SetString("LastChestOpenDay2", lastchestopen.ToString());
        ChestButton.interactable = false;
        if (PlayerPrefs.GetInt("ValueOnceSet") == 1)
        {
    ...
    private bool isChestReady()
    {
       ...
       // Debug.Log(SecondLeft);
        if (SecondLeft < 0)
        {
            ...
            PlayerPrefs.SetInt("ValueOnceSet", 1);
            return true;
        }
